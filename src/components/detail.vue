    ...
    <div class="bd-width">
      <div class="bd-left flex1">
        <div class="news-content">
          <h2 id="news_title">{{detailData.title}}</h2>
          <div class="dl-author">
            <div class="f-fl" id="newsinfo">
              <span> 发布时间：<i>{{detailData.publishTime}}</i></span>
              <span>作者：<i>{{detailData.createUsername}}</i></span>
              <!-- <span>阅读次数：<i>1167</i></span> -->
            </div>
          </div>
          <div class="dl-news" v-html="unescapeHtml(detailData.content)">
...
export default {
  ...
  created() {
    ...
    console.error('2222',JSON.parse(detailData));
