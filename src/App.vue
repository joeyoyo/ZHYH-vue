    ...
    <el-container>
      ...
      <el-header style="height: auto;background-color:#fff;" class="fixed-header">
        <el-row>
          <el-col :span="24"  >
            <el-row>
              <el-col :xs="8" :sm="6" :md="4" :lg="2" :xl="2" class="banner">
                <!-- <div class="logo">
                  <img src="./assets/zh/logo.png" alt="" />
                  <div class="logo-txt">新疆老龄基金会</div>
                </div> -->
              </el-col>
            </el-row>
            <el-row>
              ...
              <el-col >
                <div class="header_nav">
                  <el-menu class="hidden-sm-and-down" mode="horizontal" background-color="#d5bb8c" active-text-color="#c22c30">
                    <template v-for="yylist in menulist">
                      <el-submenu v-if="yylist.children && yylist.children.length > 0" :index="yylist.id + ''" :key="yylist.id" >
                        ...
                        <template v-for="eelist in yylist.children">
                          <template v-if="eelist.children.length == 0">
                            <el-menu-item v-if="yylist.children && yylist.children.length > 0"  :index="eelist.id + ''" :key="eelist.id " @click="subMenuTag(yylist.path,eelist)">
                              {{ eelist.menuName }}
														</el-menu-item>
                          </template>
                        </template>
                      ...
                      <el-menu-item v-if="yylist.children && yylist.children.length == 0"  :index="yylist.id  + ''" :key="yylist.id"  @click.self="menuTag(yylist.path,yylist)">
                        {{ yylist.menuName }}
                      </el-menu-item>
                    </template>
                  </el-menu>
    ...
    getMenuList()
      .then((res) => {
        if (res.status.code === 200) {
          ...
          let arr = res.data.filter(item => item.menuName !== '基金会活动'); //隐藏基金会活动
          console.log(arr)
					this.menulist = arr
...
.banner{
  ...
  /* height: 170px; */
  height: 240px;
  background: url('./assets/zh/banner2.png');
